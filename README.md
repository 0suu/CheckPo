...
<storage-root>/
  ...
  repos/<project-id>/
    ...
    inventory/snapshots/states/<state-id>.inventory
    inventory/snapshots/sets/roots/<set-root-id>.setroot
    inventory/snapshots/sets/leaves/ab/<set-leaf-id>.setleaf
...
再生成可能なSQLite索引は、repository配下ではなくOSのprivate user-dataに保存します。
...
将来のバックアップ・端末間転送で持ち運ぶ永続データ（portable set）は `repo.json`、`refs/`、`inventory/`、`snapshots/`、`manifests/`、`objects/` です。いずれかを省くとcheckpointを復元できません。user-dataの`derived-indexes/`は再構築可能、repository内の`journals/`、`quarantined-journals/`、`tmp/`、`locks/`はその端末での処理・復旧専用です。`quarantined-journals/`はcheckpointの一部ではありませんが、復旧できなかった作業の`journal`、`backup`、`staged`を保持するため、ユーザーが状態を確認するまで削除しません。
...
将来のクラウド対応は、上記のportable setだけを専用APIで転送し、端末ローカルの `indexes/`、`journals/`、`quarantined-journals/`、`tmp/`、`locks/` を同期しない方式で追加します。content-addressedなSnapshot v2 root、manifest chunk、object、inventory state/set root/set leafはimmutable blobとして転送できます。可変な`inventory/snapshots/head`と`refs/`にはクラウド側のcompare-and-swap、競合解決、公開順序が別途必要であり、現行版はクラウド同期そのものを実装していません。
