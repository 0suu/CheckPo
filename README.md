...
`projectId` は checkpoint lineage の ID です。Unity project の物理 path は identity ではなく、移動・リネームされる前提で扱います。storage root は user data dir 側の registry で管理します。checkpoint 内容の正本は external storage の `snapshots/` と `objects/` で、`repo.json` と `refs/` はその形式、最新位置、変更後の表示名を保持する永続メタデータです。SQLite は再構築可能な derived index であり、壊れても checkpoint / restore / discard の正本にはしません。
...
将来のバックアップ・端末間転送で持ち運ぶ永続データは `repo.json`、`refs/`、`snapshots/`、`objects/` です。`indexes/` は再構築可能、`journals/`、`quarantined-journals/`、`tmp/`、`locks/` はその端末での処理・復旧専用です。`quarantined-journals/` はチェックポイントの一部ではありませんが、復旧できなかった作業の `journal`、`backup`、`staged` を保持するため、ユーザーが状態を確認するまで削除しません。
...
V1 はクラウドバックアップ・同期に対応しません。Dropbox、Google Drive、OneDrive などの同期フォルダを storage root に指定し、複数端末から同時利用する運用も非対応です。ファイル同期サービスは CheckPo の repository lock や可変な `refs/` を協調制御しないため、競合や途中状態を作る可能性があります。

将来のクラウド対応は、上記の持ち運び可能な永続データだけを専用APIで転送し、端末ローカルの `indexes/`、`journals/`、`quarantined-journals/`、`tmp/`、`locks/` を同期しない方式で追加します。V1で作成したcheckpointは、既存のsnapshot/object IDと保存形式を変えずに転送対象へできます。
