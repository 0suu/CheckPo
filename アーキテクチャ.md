  ...
  repos/
    <project-id>/
      ...
      inventory/
        snapshots/
          head
          states/<state-id>.inventory
          sets/
            roots/<set-root-id>.setroot
            leaves/ab/<set-leaf-id>.setleaf
      manifests/
        v2/
          nodes/ab/<node-id>
          leaves/ab/<leaf-id>
...
- 端末ローカル状態: user-dataの`derived-indexes/<projectId>/`、repository内の`journals/`、`quarantined-journals/`、`tmp/`、`locks/`。`derived-indexes/`は再構築でき、その他はその端末での進行中操作または復旧専用です。
...
8. rootを `snapshots/v2/.../<root-id>.root` へatomic publishする。このfinal pathの存在がcheckpointのcommit pointである。
